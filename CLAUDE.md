...
5. training cutoff 검증: Llama-3.1-8B는 LLMLagBench 리더보드에서 완료(선언 2023-12/탐지 2023-03),
   Qwen2.5는 관리자에게 평가 요청(질문셋 비공개), Olmo3은 코퍼스 날짜 메타데이터로 직접 확인.
   **LCB-post 경계는 §4.2의 사전 지정 규칙** — arm별 보수적 상한(코퍼스 메타데이터 > 검증된
   선언 > 선언 > 출시일)의 최댓값, 잠정 Qwen2.5 출시일 2024-09 vs Olmo3 코퍼스 종료 중 늦은 쪽.
   요청 무응답이어도 설계는 자립 (출시일 상한으로 완화될 뿐). **코퍼스 등급 미만 arm은 상·하한
