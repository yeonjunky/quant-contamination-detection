...
def test_cache_roundtrip(tmp_path):
    ...
    key = CacheKey(model_name="m", quant="fp16", item_id="x", is_greedy=True, sample_id=0, prompt="p")
...
def test_cache_key_digest_changes_with_prompt():
    k1 = CacheKey(model_name="m", quant="fp16", item_id="x", is_greedy=True, sample_id=0, prompt="prompt A")
    k2 = CacheKey(model_name="m", quant="fp16", item_id="x", is_greedy=True, sample_id=0, prompt="prompt B")
...
def test_sample_item_shape(tmp_path):
    ...
    result = sample_item(model, cache, model_name="mock", quant="fp16", item_id="x", prompt="p", n_samples=5)
...
def test_sample_item_uses_cache_on_second_call(tmp_path):
    ...
    first = sample_item(model, cache, model_name="mock", quant="fp16", item_id="x", prompt="p", n_samples=3)
    ...
    second = sample_item(model_after, cache, model_name="mock", quant="fp16", item_id="x", prompt="p", n_samples=3)
...
def test_greedy_generation_is_deterministic_across_runs(tmp_path):
    ...
    r1 = sample_item(model, cache1, model_name="mock", quant="fp16", item_id="x", prompt="p", n_samples=2)
    r2 = sample_item(model, cache2, model_name="mock", quant="fp16", item_id="x", prompt="p", n_samples=2)
