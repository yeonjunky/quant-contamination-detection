...
*known* true precision:exposure_proxy interaction coefficient and confirm the
        ...
        for m in range(n_models):
            for precision in ("bf16", "quant"):
                ...
                logit = (
                    ...
                    + true_exposure_proxy * exposure_proxy[i]
                    + true_interaction * is_quant * exposure_proxy[i]
                )
                p = 1 / (1 + np.exp(-logit))
                rows.append(
                    {
                        ...
                        "model": f"model{m}",
                        "precision": precision,
                        "exposure_proxy": bool(exposure_proxy[i]),
