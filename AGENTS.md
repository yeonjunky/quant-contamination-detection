...
| Q2 주 LCB 대비 MDE (비페어링 p=0.5) | 공통 대조 n=182, 의심 n→∞ → **14.7%p**; 의심 n=873 → **16.1%p**. 실제 arm별 의심 n은 더 작음 |
...
8. **아키텍처는 통제 축이 아니다.** Qwen2.5와 Llama-3.1은 둘 다 dense GQA+RoPE라 그 열은 아무
   정보도 담지 않는다. 모델 간 남는 축은 **크기뿐**이다. 코퍼스 투명성은 Olmo arm에서 직접
   양성 일치 증거를 확보하게 하는 운영상 선택 속성이지 모델 간 비교 축이 아니다. (2026-08-05: 70B 컴퓨트 제약으로
   ...
   전부 **-Instruct 변형** 확정 (2026-08-05, provenance (f)). 이에 따라 Olmo3 코퍼스 참조
...
10. **확증 검정군은 §4.5.6의 4개뿐** (C1–C3: bf16→nf4 Q1a 탐지기별 이동 @LCB, C4: Q1b 계열 순위
   ...
   바꾸는 것은 금지. CDD가 본 연구에서 우연 수준이어도 C4를 사후 제거하지 않는다. 확증 검정
   사이징은 α/4 승수 3.339 기준 (d=0.3 → ≈124문항).
...
6. TRACER(arXiv:2605.24079)로 잔여 오염의 확인 양성을 탐색한다. 공개 코드 없음이 확인되어
   재구현 필요(프롬프트·임계값은 원문 부록에 공개, 7차 검토 부록 A 참조). Olmo3 코퍼스 검색은
   재구현의 양성 사례 거동을 기술적으로 대조하되 `no-match-found`를 음성 정답으로 쓰지 않는다
7. 구현 검증: 로컬 합성 드라이런과 제한된 H100 스모크 테스트로 로딩, 양자화 호환성, 스키마,
   유한 로그확률, 샌드박스, 메모리와 처리량만 확인한다. 출력은 연구 데이터와 분리하고 효과 크기,
   AUC, pass rate, 상관 또는 검정력을 계산하지 않는다
8. 운영 설정을 기록·고정한 뒤 본 실험을 실행한다. **본 실행만 연구 데이터의 출처**이며 C1–C4와
   계획 문항 집합은 구현 검증 결과로 바뀌지 않는다
